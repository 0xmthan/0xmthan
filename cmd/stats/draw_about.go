...
func drawAbout(cfg About) string {
	...
	p.WriteString(`<g opacity="0">` + fade(dName) +
		label(I(0), I(yName), cfg.Name, nameFS,
			opts{cls: "e-f", extra: ` font-weight="600"`}) + `</g>`)

	if cfg.Title != "" {
		tx := advance(cfg.Name, nameFS) + 14
		p.WriteString(`<g opacity="0">` + fade(dTitle) +
			label(F(tx), I(yName), cfg.Title, 13, opts{}) + `</g>`)
	}

	for i, line := range cfg.Bio {
		p.WriteString(`<g opacity="0">` + fade(delay(dBio, 0.08, i)) +
			label(I(0), I(bioStart+i*bioLH), line, 12, opts{}) + `</g>`)
	}
	...
	if len(cfg.Facts) > 0 {
		...
		for i, f := range cfg.Facts {
			...
			p.WriteString(`<g opacity="0">` + fade(delay(dFacts, 0.06, row)) +
				kv(x, I(fy), f[0], f[1], 11) + `</g>`)
		}
	}

	for i, t := range tagRows {
		p.WriteString(fmt.Sprintf(`<g opacity="0">%s`+
