...
func drawYear(s Summary) string {
	...
	p.WriteString(label(I(padL), I(16), "THE YEAR", 9,
		opts{cls: "m-f", extra: ` letter-spacing="1.3"`}))
	p.WriteString(label(I(padL), I(32), strconv.Itoa(s.Active)+" of "+
		strconv.Itoa(totalDays)+" days had a contribution", 11, opts{}))
	p.WriteString(`</g>`)

	lx := Width - 6
	p.WriteString(`<g opacity="0">`)
	p.WriteString(fade(1.30))
	p.WriteString(label(I(lx-78), I(32), "less", 9, opts{cls: "m-f", anchor: "end"}))
	fmt.Fprintf(&p, `<text xml:space="preserve" x="%d" y="32" class="d-f" `+
		`font-size="%s">%s</text>`, lx-72, f2s(fs),
		strings.Join(ramp[1:], " "))
	p.WriteString(label(I(lx), I(32), "more", 9, opts{cls: "m-f", anchor: "end"}))
	p.WriteString(`</g>`)
	...
	for r := range 7 {
		...
		fmt.Fprintf(&p, `<clipPath id="%s"><rect x="%d" y="%s" `+
		...
		fmt.Fprintf(&p, `<g clip-path="url(#%s)"><text `+
